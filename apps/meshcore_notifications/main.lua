...
-- channel NAME (slots shift when channels are added/removed). DMs always
-- alert. The master sound/blink switches cover every protocol and live in
-- Settings > Alerts.
content:Label { text = "-- Channels --", w = lvgl.PCT(100), h = 16 }

local MODE_NAMES = { [0] = "Off", [1] = "Mentions", [2] = "All" }

local channels = _mesh_get_channels() or {}
if #channels == 0 then
