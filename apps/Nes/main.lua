...
local ACTIONS = {
    { id="up",     label="Up",     nes=NES.UP,     key1=KEYS.w,     key2=KEYS.TrkUp  },
    { id="down",   label="Down",   nes=NES.DOWN,   key1=KEYS.s,     key2=KEYS.TrkDn  },
    { id="left",   label="Left",   nes=NES.LEFT,   key1=KEYS.a,     key2=KEYS.TrkLt  },
    { id="right",  label="Right",  nes=NES.RIGHT,  key1=KEYS.d,     key2=KEYS.TrkRt  },
    { id="btn_a",  label="A btn",  nes=NES.A,      key1=KEYS.m,     key2=KEYS.TrkClk },
    { id="btn_b",  label="B btn",  nes=NES.B,      key1=KEYS.n,     key2=nil         },
    { id="start",  label="Start",  nes=NES.START,  key1=KEYS.Enter, key2=nil         },
    { id="select", label="Select", nes=NES.SELECT, key1=KEYS.BkSpc, key2=nil         },
}

local bindings = {}  -- bindings[action_id] = {nes=N, key1=N|nil, key2=N|nil}

local function load_defaults()
    bindings = {}
    for _, a in ipairs(ACTIONS) do
        bindings[a.id] = { nes = a.nes, key1 = a.key1, key2 = a.key2 }
    end
end
load_defaults()
...
local function build_keymap_string()
    ...
    for _, a in ipairs(ACTIONS) do
        ...
        if b and (b.key1 or b.key2) then
            local s = string.format("%02X=", b.nes)
            if b.key1 then
