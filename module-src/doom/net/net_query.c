...
#include "net_tdeck.h"   // MESHPUNK: LAN queries ride the UDP transport
...
// MESHPUNK: securedemo_start_message removed with the secure-demo functions.
...
void NET_Query_Init(void)
{
    if (query_context == NULL)
    {
        ...
        NET_AddModule(query_context, &net_udp_module);   // MESHPUNK
...
static void formatted_printf(int wide, char *s, ...)
{
    ...
    va_start(args, s);
    i = vfprintf(stdout, s, args);   // MESHPUNK: vprintf is not a host export
    va_end(args);
...
// MESHPUNK: BlockForPacket, NET_StartSecureDemo and NET_EndSecureDemo
// removed — the master-server secure-demo feature (aes_prng, not vendored).
