...
// MESHPUNK: NET_CL_Connect's timeout and an optional per-iteration poll
// (return false to abort) — set by net_tdeck_wait.c around a connect.
int net_connect_timeout_ms = 5000;
boolean (*net_connect_poll)(void) = NULL;
...
boolean NET_CL_Connect(net_addr_t *addr, net_connect_data_t *data)
{
    ...
    while (client_connection.state == NET_CONN_STATE_CONNECTING)
    {
        ...
        // MESHPUNK: the T-Deck wait screen drives a longer, cancellable
        // connect (net_connect_timeout_ms / net_connect_poll); defaults
        // reproduce the original 5s.
        if (nowtime - start_time > net_connect_timeout_ms
         || (net_connect_poll != NULL && !net_connect_poll()))
        {
            break;
        }
...
{
    // MESHPUNK: doomgeneric's config table has no player_name variable
    // (and no M_BindStringVariable); the name stays NET_CL_Init's default.
}
