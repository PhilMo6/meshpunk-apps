...
typedef struct
{
    ...
    void *data;
    uint32_t time;      // MESHPUNK: sample count, not us (see opl_queue.h)
} opl_queue_entry_t;
...
void OPL_Queue_AdjustCallbacks(opl_callback_queue_t *queue,
                               uint32_t time, float factor)
{
    // MESHPUNK: i32<->float only — native single-precision FPU ops on the
    // ESP32-S3; the upstream i64/u64<->float round trip pulls libgcc
    // helpers (__floatdisf and friends) that are not host exports.
    int32_t offset;
    int i;

    for (i = 0; i < queue->num_entries; ++i)
    {
        offset = (int32_t)(queue->entries[i].time - time);
        queue->entries[i].time = time + (uint32_t)((float)offset * factor);
    }
}
