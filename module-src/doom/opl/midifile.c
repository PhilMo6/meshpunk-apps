...
// MESHPUNK: parses from a memory buffer via memio instead of stdio FILE*
// (the original wrote a temp .mid to disk; the module has no tmp files and
// no fgetc host export). MIDI_LoadFile is replaced by MIDI_LoadBuffer.

// MESHPUNK: this doomgeneric fork's i_swap.h has no SDL byte-swap macros;
// the ESP32-S3 is little-endian, so big-endian file fields always swap.
#ifndef SDL_SwapBE16
#define SDL_SwapBE16(x) ((unsigned short)((((unsigned short)(x) & 0x00ffu) << 8) \
                                        | (((unsigned short)(x) & 0xff00u) >> 8)))
#endif
#ifndef SDL_SwapBE32
#define SDL_SwapBE32(x) ((unsigned int)((((unsigned int)(x) & 0x000000ffu) << 24) \
...
// MESHPUNK: replaces MIDI_LoadFile. The parser copies everything it keeps
// (header by mem_fread, event data via ReadByteSequence mallocs), so the
// caller may free `buf` as soon as this returns.

midi_file_t *MIDI_LoadBuffer(void *buf, size_t buflen)
{
    midi_file_t *file;
    MEMFILE *stream;
    ...
    // Open a read stream over the caller's buffer
