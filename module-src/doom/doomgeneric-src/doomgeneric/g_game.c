...
static boolean *mousebuttons;  // = &mousearray[1], set at runtime (PIC reloc fix)
...
static boolean *joybuttons;		// = &joyarray[1], set at runtime (PIC reloc fix)
...
void G_BuildTiccmd (ticcmd_t* cmd, int maketic)
{
    ...
    // PIC reloc fix: static pointer = &array[1] loses the +1 offset
    if (!mousebuttons) {
        volatile uintptr_t mb = (uintptr_t)mousearray;
        mousebuttons = (boolean*)(mb + sizeof(boolean));
        volatile uintptr_t jb = (uintptr_t)joyarray;
        joybuttons = (boolean*)(jb + sizeof(boolean));
    }
