...
/// MESHPUNK: growable array that can also BORROW caller-owned memory.
///
/// Replaces std::vector for Object's vertex and triangle storage so a mesh
/// loaded from a baked .jmsh blob can be used IN PLACE: the loader hands the
/// Object pointers into the blob instead of copying every element into a
/// vector, which halved peak memory and deleted per-element parsing from the
/// load path. The subset of the vector interface every existing call site
/// uses is provided (size/empty/operator[]/begin/end/reserve/push_back), so
/// authored meshes keep working unchanged — push_back on a borrowed span is
/// the one illegal operation and converts it to owned by copying first.
/// Non-copyable on purpose: accidental Object copies would double-free.
template <typename T>
class JetSpan {
public:
    JetSpan() = default;
    JetSpan(const JetSpan&) = delete;
    JetSpan& operator=(const JetSpan&) = delete;
    ~JetSpan() { release(); }
    ...
    /// Point at caller-owned memory. The span does not free it; whoever owns
    /// the enclosing blob does (see Object::adoptBlob).
    void borrow(T* p, size_t n) { release(); data_ = p; count_ = n; cap_ = n; owned_ = false; }

    void reserve(size_t n) {
        if (n <= cap_ && owned_) return;
        grow(n);
    }

    void push_back(const T& v) {
        if (count_ == cap_ || !owned_) grow(count_ ? count_ * 2 : 8);
        data_[count_++] = v;
    }
...
private:
    void grow(size_t n) {
        ...
        if (n == 0) n = 8;
        T* nd = new T[n];
        for (size_t i = 0; i < count_; ++i) nd[i] = data_[i];
        if (owned_) delete[] data_;
        data_ = nd; cap_ = n; owned_ = true;
    }
    void release() {
        if (owned_ && data_) delete[] data_;
        data_ = nullptr; count_ = 0; cap_ = 0; owned_ = true;
    }
...
/// @brief MESHPUNK: which depth of a triangle drives the painter's sort.
///
/// FAST_Z gives every triangle ONE sort depth. For a small triangle any
/// representative works, but a LARGE one at a glancing angle is badly served
/// by its average: a floor tile can average NEARER than a small object
/// standing on it and paint over it. No sort order fixes that in general —
/// correct visibility varies per pixel — but the choice of representative
/// decides which way the error falls.
enum class SortDepth {
    /// Mean of the three vertex depths. Correct-on-average; the default.
    AVERAGE,
    /// Farthest vertex. A surface sorts by its BACK edge, so anything sitting
    /// on it always draws in front — the fix for ground planes and terrain,
    /// at zero cost (a max instead of a mean, and no division).
    ///
    /// Do NOT set this on a surface that should HIDE things behind it: a wall
    /// sorted by its far edge loses to geometry it ought to occlude. It is a
    /// per-object opt-in for exactly that reason.
    FARTHEST,
};
...
public:
    ...
    // MESHPUNK: JetSpan instead of std::vector so a baked mesh blob can be
    // used in place (see JetSpan above). Authored geometry still push_backs.
    JetSpan<Vertex> vertices;           ///< Mesh vertices.
    std::vector<int> indices;           ///< Index buffer (unused by the default rasteriser).
    JetSpan<Triangle> triangles;        ///< Mesh triangles.

    /// MESHPUNK: when the vertex/triangle spans borrow from a loaded blob,
    /// the Object owns the blob itself (a single malloc) and frees it on
    /// destruction. Borrow the spans BEFORE adopting; nothing may push_back
    /// into a borrowed span afterwards without accepting the copy-to-owned
    /// conversion it performs.
    void adoptBlob(void* blob) { meshBlob = blob; }
    ~Object();

private:
    void* meshBlob = nullptr;
public:
    ...
    SortDepth sortDepth = SortDepth::AVERAGE;               ///< MESHPUNK: painter's-sort depth policy.
    ...
    /// Set per frame by Scene::prepareFrame's cull-group pass: true while the
    /// object's group is fully outside the frustum. The object walk drops the
    /// object before any per-object work. Objects in no group stay false.
    bool groupCulled = false;

    /// MESHPUNK: explicit rotation-matrix override, for rig bones. When set,
    /// renderObject loads its object matrix from `matrix` (row-major, unit
    /// scale — NOT FIXED_POINT_SCALE) instead of composing the Euler
    /// `rotation` fields, so animation code can hand a parent*child*quat
    /// composition straight in without an Euler round-trip. Position still
    /// comes from `position`. cullObject falls back to its rotation-invariant
    /// bounding-sphere test (the Euler-based 8-corner refinement would be
    /// stale), which is conservative: never culls a visible bone.
    bool  useMatrix = false;
    float matrix[9] = { 1, 0, 0,  0, 1, 0,  0, 0, 1 };

    /// MESHPUNK: triangles this object contributed to the render queue on the
    /// last prepareFrame(). 0 while enabled means every triangle was rejected —
    /// backface/frontface culling, the near/far Z cull, or off-screen bounds —
    /// which distinguishes "culled away" from "drawn but not visible".
    int lastEmitted = 0;

    /// MESHPUNK: LOD level the Scene selected for this object on the last
    /// prepareFrame(). 0 = the head mesh, N = lodMeshes[N-1], -1 = the object
    /// was culled before selection. Diagnostic: distinguishes "the LOD chain
    /// is not being used" from "it is, and the coarse mesh looks wrong".
    int lastLodLevel = 0;
