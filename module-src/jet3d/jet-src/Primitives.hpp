...
/// @param material Material applied to every face.
/// @return Newly-allocated Object owned by the caller.
/// @param rings MESHPUNK: vertical subdivisions of the barrel. 0 keeps the
///        original behaviour of using `segments` for BOTH the angular and the
///        vertical count — which costs segments^2*2 triangles when a smooth
///        barrel only ever needs the angular resolution. A horizon ring wants
///        rings = 1.
Object* createCylinder(int32_t radius, int32_t height, int32_t segments, bool closedCaps, Material* material, int32_t rings = 0);
