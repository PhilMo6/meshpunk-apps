...
public:
    ...
    /// @brief Sample at a chosen mip level. The rasteriser picks the level once
    ///        per triangle from its UV-per-screen-pixel rate.
    uint16_t getColor(Vector2 uv, int mipLevel);
};
