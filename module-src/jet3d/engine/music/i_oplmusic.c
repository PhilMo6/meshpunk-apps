...
// MESHPUNK: opl_driver_ver_t lives in i_sound.h in upstream chocolate-doom
// 2.2.1; this doomgeneric fork's i_sound.h predates it, so define it here.
// Nothing in this port calls I_SetOPLDriverVer — the default (opl_v_new,
// Doom 1.9 DMX behaviour) is always used.
typedef enum
{
    opl_v_old,
    opl_v_new,
...
// MESHPUNK: mixer pump for -nosfx runs, wired as this module's Poll so music
// plays without the SFX module. A no-op in jet3d, where jet_audio always
// mixes and the music is already being pulled.
...
static boolean LoadInstrumentTable(void)
{
    ...
    // MESHPUNK: degrade gracefully (SFX-only) instead of I_Error'ing
    // inside W_CacheLumpName if the WAD has no GENMIDI lump.
    if (W_CheckNumForName("GENMIDI") < 0)
    {
        fprintf(stderr, "OPL: no GENMIDI lump in WAD, music disabled.\n");
        return false;
    }
...
// MESHPUNK: fully in-memory — upstream wrote a temp .mid to disk and
// parsed it back; here the MUS lump is converted with mus2mid() into a
// memory stream and handed straight to MIDI_LoadBuffer (which copies
// everything it keeps, so both streams are closed before returning).
...
static void *I_OPL_RegisterSong(void *data, int len)
{
    ...
    else
    {
        ...
        if (mus2mid(instream, outstream) == 0)
        {
            ...
            result = MIDI_LoadBuffer(outbuf, outbuf_len);
        }
...
music_module_t music_opl_module =
{
    ...
    I_TDeck_MusicPoll,  // MESHPUNK: pumps the mixer when running -nosfx
};
