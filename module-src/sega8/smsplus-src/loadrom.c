...
/* MESHPUNK: coleco_bios.h removed. It vendored an 8KB ColecoVision BIOS
 * dump, which is proprietary content we do not have the right to ship.
 * Colecovision is not a supported system here (it is the only one of this
 * core's targets that requires a BIOS), so the option.console == 6 branch
 * below is disabled rather than left pointing at a missing symbol. */
#include "shared.h"
...
void set_rom_config(void)
{
  ...
  /* MESHPUNK: option.console == 6 (Colecovision) removed with the BIOS. */
...
int load_rom_file(const char *filename)
{
  ...
  {
    option.console = 0;  /* MESHPUNK: Colecovision unsupported (no BIOS) */
  }
