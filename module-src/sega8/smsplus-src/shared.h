...
#define IRAM_ATTR
/* MESHPUNK: upstream stubs this to 0 off-retro-go, which silently disables
 * the per-game database in loadrom.c -- every CRC compares equal to nothing,
 * so the Codemasters/Korean mapper fixes, forced PAL timings and GG-in-SMS
 * compatibility entries never apply. Implemented in main_tdeck.c. */
extern uint32 meshpunk_crc32(uint32 crc, const uint8 *buf, uint32 len);
#define crc32_le(a, b, c) meshpunk_crc32(a, b, c)
#endif
