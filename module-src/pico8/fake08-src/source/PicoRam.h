...
// offset/time are float (not double): values stay in 0..32 with increments
// >= 1/(183*255), well above float ulp there. Doubles are software-emulated
// on Xtensa LX7 and these fields are advanced per sample in the synth.
struct musicChannel {
    int16_t count = -1;
    int16_t pattern = -1;
    uint8_t mask = 0;
...
struct sfx_state {
    ...
    int8_t prev_key = 0;
    float prev_vol = 0;
    // memoized "last non-silent note" scan (see update_sfx_state) —
    // recomputed when the sfx id or the current note row changes
    int16_t scan_sfx = -1;
    int8_t scan_note = -1;
    uint8_t scan_last_note = 0;
};
