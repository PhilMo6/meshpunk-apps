...
/* T-Deck debug trap: carts deterministically read fields as if the table
** prefix was dropped (s.player resolving to the global player). Trace every
** name resolution and field selection in the suspect source-line window so
** the compile decisions at the failing site are visible on serial.
** Remove with FAKE08_PARSE_TRACE once root-caused. */
#define FAKE08_PARSE_TRACE 0
#if FAKE08_PARSE_TRACE
#include <stdio.h>
#define PTRACE_LO 285
#define PTRACE_HI 330
#endif
...
#if FAKE08_PARSE_TRACE
    if (ls->linenumber >= PTRACE_LO && ls->linenumber <= PTRACE_HI)
      printf("[parse] L%d var '%s' GLOBAL\n", ls->linenumber, getstr(varname));
#endif
  }
#if FAKE08_PARSE_TRACE
  else if (ls->linenumber >= PTRACE_LO && ls->linenumber <= PTRACE_HI)
    printf("[parse] L%d var '%s' k=%d info=%d\n",
           ls->linenumber, getstr(varname), (int)var->k, var->u.info);
#endif
}
...
static void fieldsel (LexState *ls, expdesc *v) {
...
#if FAKE08_PARSE_TRACE
  {
    ...
    codestring(ls, &key, fname);
    if (ls->linenumber >= PTRACE_LO && ls->linenumber <= PTRACE_HI)
      printf("[parse] L%d .field '%s' (base k=%d info=%d)\n",
             ls->linenumber, getstr(fname), (int)v->k, v->u.info);
  }
#else
  checkname(ls, &key);
#endif
