...
/* MESHPUNK: provided by osd_tdeck.c — there is no timer ISR or sound thread
** in the T-Deck module. osd_pump_ticks() advances nofrendo_ticks from wall
** time, osd_check_exit() reports the firmware's exit chord, osd_do_audio()
** pulls one frame of APU output per emulated frame. */
extern void osd_pump_ticks(void);
extern int  osd_check_exit(void);
extern void osd_do_audio(void);
