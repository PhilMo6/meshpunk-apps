...
//   }
  ...
  /* MESHPUNK: derived rather than the literal 888*60 upstream used. The PSG
     builds dClock from this rate while gwenesis_SN76489_run() steps it by the
     divisor, so the two have to describe the same sample rate -- a divisor
     change with the literal left behind detunes every PSG channel. */
  gwenesis_SN76489_Init(3579545,
                        MCYCLES_PER_FRAME_NTSC * 60 / AUDIO_FREQ_DIVISOR,
                        AUDIO_FREQ_DIVISOR);
