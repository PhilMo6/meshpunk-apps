...
/* ======================================================================== */
  ...
  #endif
  /* MESHPUNK: const-table build dispatches through the pointers directly. */
  #define M68KI_DISPATCH(op) m68ki_instruction_jump_table[op]()
#else
...
/* MESHPUNK: a 16-bit index into a compact handler array, not a 64K array of
   function pointers. There are only 1784 distinct handlers, so a pointer per
   opcode spent 4 bytes to say what fits in 2: 256KB of PSRAM walked at random,
   once per instruction, behind a 32KB data cache both cores share. At 2 bytes
   an entry a 32-byte cache line covers 16 opcodes instead of 8, and code
   executes clusters of related opcodes -- same instruction, different register
   or addressing mode, which differ only in the low bits -- so twice the
   entries per line is a real reduction in misses, not just a smaller table.
   The 3.5KB slot array it redirects through stays resident.

   Dispatch goes through M68KI_DISPATCH so the upstream const-table build (no
   BUILD_TABLES) keeps working unchanged. */
#define M68K_HANDLER_SLOTS 2048

static void   (*m68ki_handler_slots[M68K_HANDLER_SLOTS])(void);
static uint16_t m68ki_instruction_index[0x10000];
static int      m68ki_slot_count;

#define M68KI_DISPATCH(op) m68ki_handler_slots[m68ki_instruction_index[op]]()

/* Called once per opcode_handler_struct at table build, not per opcode, so
   the linear scan runs ~1784 times rather than 65536. */
static uint16_t m68ki_intern_handler(void (*fn)(void))
{
  int s;
  for(s = 0; s < m68ki_slot_count; s++)
    if(m68ki_handler_slots[s] == fn)
      return (uint16_t)s;
  if(m68ki_slot_count >= M68K_HANDLER_SLOTS)
    return 0;               /* slot 0 is m68k_op_illegal; see the build below */
  m68ki_handler_slots[m68ki_slot_count] = fn;
  return (uint16_t)(m68ki_slot_count++);
}
...
static void m68ki_build_opcode_table(void)
{
  ...
  int i;
  int j;
  uint16_t h;   /* MESHPUNK: slot of the handler this entry installs */

  /* MESHPUNK: illegal is interned first so it lands in slot 0, which is both
     the default fill below and what m68ki_intern_handler() falls back to if
     the slot array were ever to overflow. */
  m68ki_slot_count = 0;
  h = m68ki_intern_handler(m68k_op_illegal);

  for(i = 0; i < 0x10000; i++)
  {
    /* default to illegal */
    m68ki_instruction_index[i] = h;
    m68ki_cycles[i] = 4;
  }
  ...
  {
    h = m68ki_intern_handler(ostruct->opcode_handler);
    for(i = 0;i < 0x10000;i++)
    {
  ...
  while(ostruct->mask == 0xff00)
  {
    ...
    for(i = 0;i <= 0xff;i++)
    {
      m68ki_instruction_index[ostruct->match | i] = h;
  ...
  while(ostruct->mask == 0xf1f8)
  {
    ...
    for(i = 0;i < 8;i++)
    {
      for(j = 0;j < 8;j++)
      {
        ...
        m68ki_instruction_index[instr] = h;
  ...
  while(ostruct->mask == 0xf1ff)
  {
    ...
    for(i = 0;i <= 0x07;i++)
    {
      m68ki_instruction_index[ostruct->match | (i << 9)] = h;
  ...
  {
    h = m68ki_intern_handler(ostruct->opcode_handler);
    m68ki_instruction_index[ostruct->match] = h;
    m68ki_cycles[ostruct->match] = ostruct->cycles * MUL;
    ostruct++;
  }
  printf("[m68k] %d handler slots of %d\n", m68ki_slot_count, M68K_HANDLER_SLOTS);
}
