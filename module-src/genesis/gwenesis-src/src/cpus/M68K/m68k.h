...
/* MESHPUNK: upstream's embedded path dereferences the masked 68000 address
 * directly -- "Direct access to ITCRAM as M68KRAM on STM32H7 mapped at 0x0" --
 * because the Game & Watch places work RAM at physical 0x0000-0xFFFF, which
 * is also why M68K_RAM is initialised to literal 0. Nothing is mapped there
 * on ESP32, so the first RAM write faults (StoreProhibited at 0xfffe).
 * Indexed through the pointer instead, matching the desktop path exactly. */
