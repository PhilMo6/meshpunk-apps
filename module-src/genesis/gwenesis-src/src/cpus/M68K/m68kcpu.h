...
/* MESHPUNK: state for the idle-loop skip in m68k_run(). Games spin on a work
   RAM flag waiting for the vblank IRQ -- on Earthworm Jim, tst.b <abs>.l plus
   beq.s was 67% of every instruction we executed. The skip needs two things to
   be true of the loop before it will short a slice:

     m68ki_wrote    -- a genuine spin only reads, so any write disqualifies it.
     m68ki_io_read  -- a read that fell through to hardware disqualifies it too.
                       Reading a VDP or I/O port can have side effects, and the
                       loop's address alone cannot tell work RAM from a port
                       (both are absolute long). Only RAM polls are skipped.
     m68ki_idle_regs -- D0-D7/A0-A7 must be UNCHANGED across the loop. Without
                       this the two guards above only prove the loop disturbs
                       nothing, not that it cannot make progress: a register
                       countdown (subq #1,d0 / bne) writes no memory and reads
                       no port, so it passed both and got skipped -- which does
                       not advance the count, it just makes the delay take ~20x
                       longer. That is what hung Sonic's SEGA screen for a
                       minute. Registers frozen + no writes + no port reads
                       means the loop genuinely cannot exit on its own, so only
                       an interrupt can end it. */
/* Runtime toggle (m68k_set_idle_skip). The skip is the largest single win in
   this port and also the part most likely to misbehave on a game we have not
   tried, so it is switchable from the launcher rather than only at build time
   -- both as a compatibility escape hatch and so its effect can be A/B'd in
   one session instead of costing a rebuild and a flash. */
/* MESHPUNK: cartridge SRAM. Gwenesis has none — the bus maps everything below
   0x800000 to ROM and drops writes there on its default case, so battery-backed
   saves were silently discarded. The window has to be tested here rather than in
   gwenesis_bus_map_address() because the read fast path above bypasses the bus
   decode entirely for anything under 0x800000.
   Range test is one unsigned compare: a below start wraps huge and fails span.
   m68ki_sram is NULL for carts without it, so the cost on every ROM fetch is a
   cached load and a predictable branch. */
static uint8_t *m68ki_sram;
static uint32_t m68ki_sram_start;
static uint32_t m68ki_sram_span;     /* end - start, inclusive */
static int      m68ki_sram_dirty;

static int m68ki_idle_enable = 1;
static int m68ki_wrote;
static int m68ki_io_read;
static uint m68ki_idle_pc;
static int m68ki_idle_hits;
static uint m68ki_idle_regs[16];
...
INLINE uint m68ki_read_8(uint address)
{
  ...
  m68ki_set_fc(FLAG_S | m68ki_get_address_space()) /* auto-disable (see m68kcpu.h) */
	...
	if (ADDRESS_68K(address) <  0x800000) {
		/* MESHPUNK: cartridge SRAM shadows ROM in its window. */
		uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
		if (m68ki_sram && o <= m68ki_sram_span) return m68ki_sram[o];
		return FETCH8ROM(ADDRESS_68K(address));
	}
	if (ADDRESS_68K(address) >= 0xFF0000) return FETCH8RAM(ADDRESS_68K(address));
	m68ki_io_read = 1;   /* MESHPUNK: hardware, not memory */
...
INLINE uint m68ki_read_16(uint address)
{
  ...
  m68ki_set_fc(FLAG_S | m68ki_get_address_space()) /* auto-disable (see m68kcpu.h) */
 	...
 	if (ADDRESS_68K(address) <  0x800000) {
		/* MESHPUNK: cartridge SRAM shadows ROM in its window. */
		uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
		if (m68ki_sram && o <= m68ki_sram_span) return ((uint)m68ki_sram[o] << 8) | m68ki_sram[o + 1];
...
INLINE uint m68ki_read_32(uint address)
{
  ...
  m68ki_set_fc(FLAG_S | m68ki_get_address_space()) /* auto-disable (see m68kcpu.h) */
	if (ADDRESS_68K(address) <  0x800000) {
		/* MESHPUNK: cartridge SRAM shadows ROM in its window. */
		uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
		if (m68ki_sram && o <= m68ki_sram_span) return ((uint)m68ki_sram[o] << 24) | ((uint)m68ki_sram[o + 1] << 16)
		             | ((uint)m68ki_sram[o + 2] << 8) | m68ki_sram[o + 3];
...
INLINE void m68ki_write_8(uint address, uint value)
{
  ...
  m68ki_set_fc(FLAG_S | FUNCTION_CODE_USER_DATA) /* auto-disable (see m68kcpu.h) */
        m68ki_wrote = 1;   /* MESHPUNK: disqualifies the idle-loop skip */
        {   /* MESHPUNK: cartridge SRAM window */
            uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
            if (m68ki_sram && o <= m68ki_sram_span) {
                m68ki_sram[o] = (uint8_t)value;
                m68ki_sram_dirty = 1;
                return;
            }
        }
...
INLINE void m68ki_write_16(uint address, uint value)
{
  ...
  m68ki_set_fc(FLAG_S | FUNCTION_CODE_USER_DATA) /* auto-disable (see m68kcpu.h) */
        ...
        {   /* MESHPUNK: cartridge SRAM window */
            uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
            if (m68ki_sram && o <= m68ki_sram_span) {
                m68ki_sram[o] = (uint8_t)(value >> 8); m68ki_sram[o + 1] = (uint8_t)value;
                m68ki_sram_dirty = 1;
                return;
            }
        }
...
INLINE void m68ki_write_32(uint address, uint value)
{
  ...
  m68ki_set_fc(FLAG_S | FUNCTION_CODE_USER_DATA) /* auto-disable (see m68kcpu.h) */
        ...
        {   /* MESHPUNK: cartridge SRAM window */
            uint32_t o = ADDRESS_68K(address) - m68ki_sram_start;
            if (m68ki_sram && o <= m68ki_sram_span) {
                m68ki_sram[o] = (uint8_t)(value >> 24); m68ki_sram[o + 1] = (uint8_t)(value >> 16);
			m68ki_sram[o + 2] = (uint8_t)(value >> 8); m68ki_sram[o + 3] = (uint8_t)value;
                m68ki_sram_dirty = 1;
                return;
            }
        }
