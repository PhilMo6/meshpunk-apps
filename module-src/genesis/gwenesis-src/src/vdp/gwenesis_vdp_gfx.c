...
/* MESHPUNK: the VDP render chain is the largest single consumer of a frame
 * (~43% measured on hardware, ~78us per scanline). Both the code and the data
 * it walks live in PSRAM behind a 16KB shared instruction cache, so nearly
 * every pattern fetch is a miss. Running the chain from internal SRAM is the
 * same escape that broke the SNES renderer wall on this silicon. The build
 * gives this file -mtext-section-literals and -fno-jump-tables so the section
 * stays self-contained. */
#define VDP_HOT __attribute__((section(".iram.text")))
...
VDP_HOT void draw_pattern_nofliph_sprite(uint8_t *scr, uint32_t p, uint8_t attrs)
{
  if (p == 0) return;
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_fliph_sprite(uint8_t *scr, uint32_t p, uint8_t attrs)
{
  if (p == 0) return;
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_nofliph_sprite_over_planes(uint8_t *scr, uint32_t p, uint8_t attrs)
{
  if (p == 0) return; 
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_fliph_sprite_over_planes(uint8_t *scr, uint32_t p, uint8_t attrs)
{
  if (p == 0) return;
 ...
 ******************************************************************************/
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_sprite(uint8_t *scr, uint16_t name, int paty) {
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_sprite_over_planes(uint8_t *scr, uint16_t name, int paty) {
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_planeB(uint8_t *scr, uint16_t name, int paty) {
 // uint16_t pat_addr = name  << 5; // * 32;
 // uint8_t pat_palette = BITS(name, 13, 2);
 // unsigned int is_pat_pri = name & 0x8000;
  //uint8_t *pattern = VRAM + pat_addr;
...
static inline __attribute__((always_inline))
VDP_HOT void draw_pattern_planeA(uint8_t *scr, uint16_t name, int paty) {
  // uint16_t pat_addr = name << 5; //* 32;
  // uint8_t pat_palette = BITS(name, 13, 2);
  // unsigned int is_pat_pri = name & 0x8000;
  // uint8_t *pattern = VRAM + pat_addr;
...
static inline __attribute__((always_inline))
VDP_HOT void draw_line_b(int line)
{
...
static inline __attribute__((always_inline))
VDP_HOT void draw_line_aw(int line) {
...
//__attribute__((optimize("unroll-loops")))
static inline __attribute__((always_inline)) 
VDP_HOT void draw_sprites_over_planes(int line)
{
    uint8_t *scr;
...
static inline __attribute__((always_inline)) 
VDP_HOT void draw_sprites(int line)
{
  uint8_t *scr;
...
/* MESHPUNK: two fixes here.
   1. CRAM565 entries are stored BYTE-SWAPPED (see gwenesis_vdp_mem.c) so the
      blit is a straight copy. SPACE/CONV only work on plain RGB565, so running
      them on swapped words mangled every colour — a whole-screen tint on any
      game that stays in H32. Swap in, interpolate, swap out.
   2. SPACE's red mask was 0xe800, which drops red bit 12 and folds in bit 11.
      It has to be 0xf800 to select bits 15-11.
   Field spacing still holds after the fix: R lands at bits 25-21, G at 15-10,
   B at 4-0, and summing four samples grows each field by 2 bits without
   reaching the next one. */
#define GSWAP(v)  ((uint16_t)(((v) >> 8) | ((v) << 8)))
#define CONV(b)   ((0x3e00000 & b)>>10) | ((0xfc00 & b)>>5) | ((0x1f & b))
#define SPACE(c)  ((0xf800 & c)<<10) | ((0x7e0 & c)<<5) | ((0x1f & c))

/* Shadow/highlight, applied to a byte-swapped entry. Halving the raw word
   would carry the low bit of one byte into the top of the other; this halves
   each channel with the usual RGB565 mask instead. */
static inline uint16_t gwen_shadow(uint16_t c)
{
  uint16_t p = GSWAP(c);
  p = (uint16_t)((p >> 1) & 0x7BEF);
  return GSWAP(p);
}
static inline uint16_t gwen_highlight(uint16_t c)
{
  uint16_t p = GSWAP(c);
  p = (uint16_t)(((p >> 1) & 0x7BEF) | 0x8410);
  return GSWAP(p);
}
...
blit_4to5_line(uint16_t *in, uint16_t *out) {
  ...
  for (int x_src = 0, x_dst = 0; x_src < 256; x_src += 4, x_dst += 5) {
    uint32_t b0 = SPACE((uint32_t)GSWAP(src_row[x_src]));
    uint32_t b1 = SPACE((uint32_t)GSWAP(src_row[x_src + 1]));
    uint32_t b2 = SPACE((uint32_t)GSWAP(src_row[x_src + 2]));
    uint32_t b3 = SPACE((uint32_t)GSWAP(src_row[x_src + 3]));

    dest_row[x_dst] = GSWAP(CONV(b0));
    dest_row[x_dst + 1] = GSWAP(CONV((b0 + b0 + b0 + b1) >> 2));
    dest_row[x_dst + 2] = GSWAP(CONV((b1 + b2) >> 1));
    dest_row[x_dst + 3] = GSWAP(CONV((b2 + b2 + b2 + b3) >> 2));
    dest_row[x_dst + 4] = GSWAP(CONV(b3));
  }
}

VDP_HOT void gwenesis_vdp_render_line(int line)
{
  mode_h40 = REG12_MODE_H40;
  //mode_pal = REG1_PAL;
    ...
    if (MODE_SHI) {
      for (int x = 0; x < screen_width; x++) {
        ...
        if ((plane & 0xC0) < (sprite & 0xC0)) {
          ...
          case 0x3E:
            screen_buffer_line[x] = gwen_highlight(CRAM565[plane]);
            break;
          // Palette=3, Sprite=15 :> draw plane, force shadow
          case 0x3F:
            screen_buffer_line[x] = gwen_shadow(CRAM565[plane]);
            break;
          // draw sprite, normal
          default:
    ...
    if (MODE_SHI) {
      for (int x = 0; x < screen_width; x++) {
        ...
        if ((plane & 0xC0) < (sprite & 0xC0)) {
          ...
          // Palette=3, Sprite=14 :> draw plane, force highlight
          case 0x3E:
            buffer_line_H32[x] = gwen_highlight(CRAM565[plane]);
            break;
          // Palette=3, Sprite=15 :> draw plane, force shadow
          case 0x3F:
            buffer_line_H32[x] = gwen_shadow(CRAM565[plane]);
            break;
          // draw sprite, normal
          default:
