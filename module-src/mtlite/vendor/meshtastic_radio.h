...
struct MeshSession {
    ...
    size_t buildTx(uint8_t channel_idx,
                    ...
                    bool want_response = false,   // MESHPUNK: was hardcoded false —
                                                  // the NodeInfo key-bootstrap
                                                  // exchange needs Data.want_response
                    uint32_t request_id = 0)      // MESHPUNK: ROUTING ACK plumbing
    {
    ...
    size_t buildDmTx(uint32_t to_node,
                      ...
                      uint8_t tx_hop_limit = MESH_HOP_RELIABLE,
                      uint32_t request_id = 0,   // MESHPUNK: ROUTING ACK plumbing
                      uint32_t *out_pkt_id = nullptr)  // MESHPUNK: reports the id used —
                                                       // the sender retry ladder matches
                                                       // ROUTING acks against it
    {
        ...
        size_t data_len = meshEncodeData(data_buf, sizeof(data_buf),
                                          portnum, payload, payload_len,
                                          false, false, request_id);   // MESHPUNK
        if (data_len == 0) return 0;

        // PKI encrypt (produces ciphertext + 12 bytes overhead)
        uint8_t encrypted[256];
        uint32_t pkt_id = id_gen.next();
        if (out_pkt_id) *out_pkt_id = pkt_id;   // MESHPUNK
    ...
    size_t buildNodeInfoTx(uint8_t channel_idx, uint32_t to,
                            ...
                            uint8_t tx_hop_limit = MESH_HOP_RELIABLE,
                            bool want_response = false)   // MESHPUNK: forwarded to
                                                          // buildTx (key bootstrap)
    {
        ...
        return buildTx(channel_idx, to, PORT_NODEINFO,
                        ...
                        tx_hop_limit, want_response);   // MESHPUNK
    }
};
