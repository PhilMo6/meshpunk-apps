...
static inline size_t meshEncodeData(uint8_t *buf, size_t capacity,
                                     ...
                                     uint32_t request_id = 0)  // MESHPUNK: field 6 —
                                                               // ROUTING ACKs carry the
                                                               // acked packet's id here
{
    ...
    if (request_id)
        if (!w.writeFixed32Field(6, request_id)) return 0;  // MESHPUNK
    if (ok_to_mqtt)
        if (!w.writeVarintField(9, 1)) return 0;  // bitfield bit 0 = ok_to_mqtt
    return w.written();
}

// MESHPUNK: position encoder, mirroring meshDecodePosition's field map
// (1 latitude_i sfixed32, 2 longitude_i sfixed32, 4 time fixed32,
// 12 precision_bits varint). precision_bits 1..31 declares a reduced-
// precision position (the coordinates should already be masked+centered by
// the caller); 0/32 = full precision, field omitted.
static inline size_t meshEncodePosition(uint8_t *buf, size_t capacity,
                                         int32_t latitude_i, int32_t longitude_i,
                                         uint32_t time,
                                         uint32_t precision_bits = 0) {
    PbWriter w = pbWriter(buf, capacity);
    if (!w.writeFixed32Field(1, (uint32_t)latitude_i)) return 0;
    if (!w.writeFixed32Field(2, (uint32_t)longitude_i)) return 0;
    if (time)
        if (!w.writeFixed32Field(4, time)) return 0;
    if (precision_bits >= 1 && precision_bits <= 31)
        if (!w.writeVarintField(12, precision_bits)) return 0;
    return w.written();
}
