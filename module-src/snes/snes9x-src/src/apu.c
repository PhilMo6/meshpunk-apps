...
/* MESHPUNK: pull-model audio. While snes_sndq_active is set, SPC700 DSP
 * writes are latched (register from the $F2 port plus the data byte) into a
 * single-producer/single-consumer ring; the firmware sound task drains the
 * ring with S9xDrainAPUDSPQueue() and applies each write through
 * S9xSetAPUDSPReg() before mixing. SoundData, APU.DSP and the soundux state
 * are owned by the sound task while the ring is active — the SPC700 side
 * only reads them (S9xGetAPUDSP status polls, which may lag by up to one
 * mix period). Ring occupancy derives from the head/tail counters only. */
#define SNDQ_SIZE 512
#define SNDQ_MASK (SNDQ_SIZE - 1)
typedef struct { uint8_t reg; uint8_t byte; } SndQEntry;
static SndQEntry s_sndq[SNDQ_SIZE];
static volatile uint32_t s_sndq_head; /* producer: SPC700 (emu task) */
static volatile uint32_t s_sndq_tail; /* consumer: sound task */
volatile int snes_sndq_active = 0;

void S9xDrainAPUDSPQueue(void)
{
   uint32_t tail = s_sndq_tail;
   while (tail != s_sndq_head)
   {
      SndQEntry e;
      /* Keep the entry load behind the head comparison. */
      __asm__ __volatile__("" ::: "memory");
      e = s_sndq[tail & SNDQ_MASK];
      S9xSetAPUDSPReg(e.reg, e.byte);
      tail++;
      s_sndq_tail = tail;
   }
}
...
void S9xSetAPUDSP(uint8_t byte)
{
   ...
   S9xSetAPUDSPReg(reg, byte);
}

void S9xSetAPUDSPReg(uint8_t reg, uint8_t byte)
{
