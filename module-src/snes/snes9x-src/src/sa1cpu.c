 ...
 * MESHPUNK: SA-1 CPU core, ported from libretro/snes9x2005 (source/sa1cpu.c,
 * commit deb49d80). Recompiles this port's cpuops.c with SA1_OPCODES: the
 * macros below rename every global the second compile would emit and remap
 * the CPU/ICPU state onto the SA1 struct; cpuops.c's own #ifndef SA1_OPCODES
 * branches drop the S-CPU cycle accounting and APU coupling. */
...
SA1_HOT void S9xSA1MainLoop()
{
   ...
   /* MESHPUNK: 12 = 3 SA-1 ops per S-CPU opcode x the 4-opcode batch in
    * cpuexec.c — the hardware clock ratio at batch granularity. */
   for (i = 0; i < 12 && SA1.Executing; i++)
   {
