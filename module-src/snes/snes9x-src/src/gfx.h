...
/* MESHPUNK fast-path renderer (fastrender.c) for BG modes 0/1;
 * S9xUpdateScreen forks to it per span when snes_fast_ok() passes. */
int snes_fast_ok(void);
void snes_fast_span(uint32_t starty, uint32_t endy, const SLineData *linedata,
                    const uint32_t *fixedc_j);
extern uint32_t snes_fixedc_j[240]; /* per-line $2132 journal (gfx.c) */
extern uint8_t snes_row_swapped[SNES_HEIGHT_EXTENDED]; /* fastrender.c */

/* MESHPUNK render-worker plumbing (gfx.c): Core 0 publishes the journals
 * per frame; the Core-1 worker consumes the latest pair and renders whole
 * frames through the fast path. ComputeClipWindows is exposed for the
...
extern volatile int snes_worker_enable;
extern int snes_obj_dirty;
extern const SOBJLines *fr_objlines;
extern const uint8_t *fr_objw;
extern const uint8_t *fr_objvis;
extern const SOBJ *fr_objs;
extern uint32_t fr_objnamebase;
extern uint32_t fr_objnamesel;
extern const struct SPalWrite_s *fr_pal_j;
extern uint32_t fr_pal_jn;
extern uint16_t fr_bg_scbase[4];
extern uint16_t fr_bg_namebase[4];
extern uint16_t fr_bg_scsize[4];
extern uint8_t fr_bg_tile16[4];
...
extern int fr_fblank;           /* span forced-blank state (fastrender.c) */
