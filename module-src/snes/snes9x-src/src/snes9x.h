...
/* MESHPUNK: fxemu.c (snes9x2010) expects these; this port dropped them.
 * The master clocks are INTEGERS on purpose — fxemu.c divides a uint64_t by
 * them, and a floating-point constant would turn that into a 64-bit int to
 * double conversion, which pulls __floatundidf. That symbol is not in
 * host_exports[], so the module would fail to load. */
#ifndef TRUE
#define TRUE  1
#endif
#ifndef FALSE
#define FALSE 0
#endif
#define NTSC_MASTER_CLOCK 21477272
#define PAL_MASTER_CLOCK  21281370
...
typedef struct
{
   ...
   bool     SuperFX;
   /* GSU instructions per scanline at 10.7MHz, with the 0.417 clocks-per-
    * instruction factor already folded in; S9xResetSuperFX scales it to the
    * cart's actual clock. Read by fxemu.c. */
   uint32_t SuperFXSpeedPerLine;
   bool     DSP1Master;
   bool     SA1;
   bool     C4;
   bool     SDD1;
