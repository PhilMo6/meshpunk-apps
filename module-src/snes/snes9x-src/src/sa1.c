 ...
 * MESHPUNK: SA-1 coprocessor, ported from libretro/snes9x2005 (source/sa1.c,
 * commit deb49d80). Local adaptations, each tagged MESHPUNK inline:
 * - The char-conversion-2 staging buffer is a static array. Upstream parked
 *   it at Memory.ROM[MAX_ROM_SIZE - 0x10000]; this port sizes the ROM buffer
 *   to the cart, so that address does not exist.
 * - BW-RAM/I-RAM stores set CPU.SRAMModified so the module's .srm autosave
 *   (snes_sram_dirty, gfx.c) sees SA-1-side saves. */
...
/* MESHPUNK: 128-byte char-conversion-2 staging area (8 x 16-byte writes in,
 * 2 x 64-byte tile reads out). See header note. */
static uint8_t sa1_cc2_buf[128];
...
SA1_HOT uint8_t S9xSA1GetByte(uint32_t address)
{
...
SA1_HOT uint16_t S9xSA1GetWord(uint32_t address)
{
   OpenBus = S9xSA1GetByte(address);
   return OpenBus | (S9xSA1GetByte(address + 1) << 8);
}

SA1_HOT void S9xSA1SetByte(uint8_t byte, uint32_t address)
{
   ...
   case MAP_LOROM_SRAM:
      ...
      CPU.SRAMModified = true; /* MESHPUNK: .srm autosave sees SA-1 writes */
      return;
   case MAP_BWRAM:
      SA1.BWRAM[(address & 0x7fff) - 0x6000] = byte;
      CPU.SRAMModified = true; /* MESHPUNK */
      return;
   case MAP_BWRAM_BITMAP:
      address -= 0x600000;
      if (SA1.VirtualBitmapFormat == 2)
...
SA1_HOT void S9xSA1SetWord(uint16_t Word, uint32_t address)
{
   S9xSA1SetByte((uint8_t) Word, address);
   S9xSA1SetByte((uint8_t)(Word >> 8), address + 1);
}

SA1_HOT void S9xSA1SetPCBase(uint32_t address)
{
   uint8_t* GetAddress = SA1.Map [(address >> MEMMAP_SHIFT) & MEMMAP_MASK];
   if (GetAddress >= (uint8_t*) MAP_LAST)
   {
...
void S9xSetSA1MemMap(uint32_t which1, uint8_t map)
{
   ...
   {
      /* Code from Snes9x 1.54.1 - This allows Super Mario World VLDC 9 hack
         to load. Conversion to int is needed here - map is promoted but
         which1 is not */
...
void S9xSetSA1(uint8_t byte, uint32_t address)
{
   ...
   case 0x220a:
      if (((byte ^ Memory.FillRAM[0x220a]) & 0x80) && (Memory.FillRAM[0x2301] & byte & 0x80))
      {
         ...
         SA1.Flags |= IRQ_PENDING_FLAG;
         SA1.IRQActive |= SNES_IRQ_SOURCE;
         SA1.Executing = !SA1.Waiting && SA1.S9xOpcodes; /* MESHPUNK: wake a WAI-parked chip */
      }
      if (((byte ^ Memory.FillRAM[0x220a]) & 0x40) && (Memory.FillRAM[0x2301] & byte & 0x40))
      {
         ...
         SA1.Executing = !SA1.Waiting && SA1.S9xOpcodes; /* MESHPUNK */
      }
   ...
   case 0x224f:
      ...
      if ((Memory.FillRAM [0x2230] & 0xb0) == 0xa0)
      {
         ...
         /* MESHPUNK: staging buffer replaces upstream's
          * Memory.ROM[MAX_ROM_SIZE - 0x10000] scratch. */
         memcpy(sa1_cc2_buf + (SA1.in_char_dma << 4), &Memory.FillRAM [0x2240], 16);
   ...
   case 0x2254:
      ...
      case 1: /* divide */
         ...
         {
            /* MESHPUNK: snes9x 1.62's divide (fixed twice upstream — 1.56
             * and 1.62 — for negative dividends; the donor's C-truncation
             * form also sign-extended the quotient over the remainder
             * bits). Biasing by divisor<<16 makes the unsigned div/mod
             * yield the wrapped signed quotient and a positive remainder. */
            ...
            SA1.sum = ((uint32_t) remainder << 16) | quotient;
         }
         SA1.op1 = 0;
         SA1.op2 = 0;
         break;
...
static void S9xSA1CharConv2(void)
{
   ...
   uint8_t* q = sa1_cc2_buf + offset * 64; /* MESHPUNK: staging buffer */
   int l, b;
...
static void S9xSA1DMA()
{
   ...
   if (Memory.FillRAM [0x2230] & 4)
   {
      ...
      len &= Memory.SRAMMask;
      d = Memory.SRAM + dst;
      CPU.SRAMModified = true; /* MESHPUNK: DMA into BW-RAM is a save write */
   }
   else
   {
      dst &= 0x3ff;
