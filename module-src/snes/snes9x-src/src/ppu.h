...
extern uint8_t snes_vram_dirty[64]; /* MESHPUNK: 1KB VRAM pages written */
...
/* MESHPUNK: palette writes that land during active display. The Core-1
 * renderer binds one frame-boundary palette, so without this record a
 * mid-frame CGRAM change (HDMA gradients, per-line recolouring) would draw
 * the whole frame with one palette. Entries are appended in line order and
 * replayed by the renderer as it descends. Storage in gfx.c. Both writes
 * below already call FLUSH_REDRAW, which is how the Core-0 path stays
 * correct: it re-reads the palette for the span it starts after the flush. */
typedef struct SPalWrite_s
{
   uint8_t  line;
   uint8_t  idx;
   uint16_t col;
...
#define SNES_PAL_J_MAX 1024

extern SPalWrite snes_pal_j[SNES_PAL_J_MAX];
extern uint32_t  snes_pal_j_count;
extern uint8_t   snes_in_active;   /* set between start/end screen refresh */
extern uint8_t   snes_cur_line;    /* RenderLine's argument; IPPU.CurrentLine
                                    * stays 0 on worker-rendered frames */

/* Bit N set if BG mode N was used on any line of the frame. The Core-1
 * renderer picks its renderer once per frame while the Core-0 path picks per
 * span, so a frame using more than one mode, or any mode but 0 and 1, is out
 * of the fast renderer's scope. Reset each frame by S9xStartScreenRefresh. */
extern uint8_t   snes_mode_mask;

/* PPU state rewritten after rendering has started, one bit per group:
 * 01 BGmode/tile size/BG3 priority  02 BGbases  04 CGRAM  08 windows
 * 10 layer-enable  20 colour-math  40 VRAM  80 OAM/sprite addressing.
 * The Core-1 renderer binds all of it once per frame, so a frame that
 * rewrites any of it (bar CGRAM, journaled per line, and VRAM, which games
 * write every frame harmlessly) has to render on Core 0 instead. */
extern uint8_t   snes_mid_mask;

static INLINE void snes_pal_note(uint8_t idx)
{
   if (!snes_in_active || snes_pal_j_count >= SNES_PAL_J_MAX)
      return;
   snes_pal_j[snes_pal_j_count].line = snes_cur_line;
   snes_pal_j[snes_pal_j_count].idx  = idx;
   snes_pal_j[snes_pal_j_count].col  = IPPU.ScreenColors[idx];
   snes_pal_j_count++;
}
