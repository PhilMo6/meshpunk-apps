...
/* MESHPUNK: per-line fixed-colour journal (packed R|G<<8|B<<16, 5-bit
 * components), recorded by RenderLine like the scroll journal. $2132
 * writes no longer flush a span — the fast renderer reads this per band,
 * so HDMA colour gradients stop fragmenting frames into one-line spans.
 * The stock path (modes 2-7) reads PPU.FixedColour* live and now sees a
 * whole span's last value on mid-frame changes. */
uint32_t snes_fixedc_j[240];

static void DrawOBJS_impl(bool OnMain, uint8_t D);
static void DrawOBJS(bool OnMain, uint8_t D)
{
   DrawOBJS_impl(OnMain, D);
}
...
/* MESHPUNK worker publish/consume: at each frame boundary Core 0 copies
 * the live journals (LineData is private to this file) into a staging
 * pair and advances the sequence; the Core-1 render worker copies the
 * latest published pair into its private buffers. The publisher writes
 * slot (seq+1)&1, so a consumer holding seq never reads a slot being
 * written. */
volatile uint32_t snes_frame_seq = 0;
volatile int snes_worker_hungry = 0;
/* Core 0 clears this for frames it renders itself. */
volatile int snes_worker_enable = 0;
/* 1KB VRAM pages written since the last publish (set by the $2118/$2119
 * write paths in ppu.h, which every VRAM write including DMA goes
 * through). Publishing copies only these — a full 64KB copy per frame
 * evicts the whole data cache and costs Core 0 far more than the copy. */
uint8_t snes_vram_dirty[64];
/* Set when S9xSetupOBJ rebuilds the sprite tables. */
int snes_obj_dirty = 1;
static SLineData s_pub_ld[240];
static uint32_t s_pub_fx[240];
static uint16_t s_pub_pal[256];
static uint8_t s_pub_rg[6];
static SOBJLines s_pub_ol[240];
static uint8_t s_pub_ow[128];
static uint8_t s_pub_ov[128];
static SOBJ s_pub_ob[128];
static uint16_t s_pub_onb, s_pub_ons;
static uint16_t s_pub_bgw[12]; /* scbase[4], namebase[4], scsize[4] */
static uint8_t s_pub_bgb[6];   /* tile16[4], bgmode, bg3prio */
static ClipData s_pub_clip[2];
static uint16_t s_pub_sh;
static uint8_t s_pub_vram[0x10000];
static SPalWrite s_pub_pal_j[SNES_PAL_J_MAX];
static uint32_t s_pub_pal_jn;

/* MESHPUNK: see ppu.h. snes_frame_pal is the palette as it stood when active
 * display began — the publish snapshot is taken at V-blank, which is the
 * END-of-frame palette, so the renderer needs this plus the journal to
 * reconstruct what each line actually saw. */
SPalWrite snes_pal_j[SNES_PAL_J_MAX];
uint32_t  snes_pal_j_count = 0;
uint8_t   snes_in_active = 0;
uint8_t   snes_cur_line = 0;
uint16_t  snes_frame_pal[256];

uint8_t snes_mode_mask = 0; /* see ppu.h */
uint8_t snes_mid_mask = 0;  /* see ppu.h */


/* Called from S9xEndScreenRefresh — the start of V-blank, which is where
 * the stock renderer drew the frame. Everything the renderer reads is
 * still display-consistent here: the game's NMI handler has not yet run
 * its VRAM/OAM uploads for the next frame. Publishing any later (after
 * S9xMainLoop returns, i.e. at the END of V-blank) hands the worker
 * next-frame sprite tiles against this frame's journals.
 *
 * The copies only run when the worker asked for a frame, so Core 0 pays
 * for the frames actually rendered, and one buffer set suffices: Core 0
 * writes it only while the worker sits idle waiting. */
void snes_worker_publish(void)
{
   int b;

   if (!snes_worker_enable || !snes_worker_hungry || !snes_fast_ok())
      return;
   ...
   for (b = 0; b < 4; b++)
   {
      s_pub_bgw[b] = PPU.BG[b].SCBase;
      s_pub_bgw[4 + b] = PPU.BG[b].NameBase;
      s_pub_bgw[8 + b] = PPU.BG[b].SCSize;
      s_pub_bgb[b] = PPU.BG[b].BGSize;
   }
   s_pub_bgb[4] = PPU.BGMode;
   s_pub_bgb[5] = PPU.BG3Priority;
   s_pub_sh = PPU.ScreenHeight;
   memcpy(s_pub_clip, IPPU.Clip, sizeof(s_pub_clip));
   memcpy(s_pub_ld, LineData, sizeof(s_pub_ld));
   memcpy(s_pub_fx, snes_fixedc_j, sizeof(s_pub_fx));
   /* Start-of-frame palette plus the writes that landed during active
    * display; IPPU.ScreenColors here is already the end-of-frame state. */
   memcpy(s_pub_pal, snes_frame_pal, 512);
   s_pub_pal_jn = snes_pal_j_count;
   if (s_pub_pal_jn)
      memcpy(s_pub_pal_j, snes_pal_j, s_pub_pal_jn * sizeof(SPalWrite));
   if (snes_obj_dirty)
   {
      memcpy(s_pub_ol, GFX.OBJLines, sizeof(s_pub_ol));
      memcpy(s_pub_ow, GFX.OBJWidths, 128);
      memcpy(s_pub_ov, GFX.OBJVisibleTiles, 128);
      memcpy(s_pub_ob, PPU.OBJ, sizeof(s_pub_ob));
      snes_obj_dirty = 0;
   }
   {
      int p;
      for (p = 0; p < 64; p++)
         if (snes_vram_dirty[p])
         {
   ...
   s_pub_ons = PPU.OBJNameSelect;
   s_pub_rg[0] = Memory.FillRAM[0x212c];
   s_pub_rg[1] = Memory.FillRAM[0x212d];
   s_pub_rg[2] = Memory.FillRAM[0x2130];
   s_pub_rg[3] = Memory.FillRAM[0x2131];
   s_pub_rg[4] = (Memory.FillRAM[0x2133] & 8) ? 1 : 0;
   s_pub_rg[5] = PPU.ForcedBlanking ? 1 : 0;
...
/* Worker side: point the span inputs at the published set. No copying —
 * Core 0 does not touch it again until the worker goes hungry. */
uint16_t snes_worker_bind(void)
{
   int b;
#if 0 /* DIAGNOSTIC: bind live state instead of the publish snapshot. Racy by
       * construction: Core 0 owns this state. Ruled out publish timing as
       * the cause of the remaining corruption. */
   fr_vram = Memory.VRAM;
   fr_pal = IPPU.ScreenColors;
   fr_pal_j = NULL;
   fr_pal_jn = 0;
   fr_objlines = GFX.OBJLines;
   fr_objw = GFX.OBJWidths;
   fr_objvis = GFX.OBJVisibleTiles;
   fr_objs = PPU.OBJ;
   fr_objnamebase = PPU.OBJNameBase;
   fr_objnamesel = PPU.OBJNameSelect;
   fr_clip = IPPU.Clip;
   for (b = 0; b < 4; b++)
   {
      fr_bg_scbase[b] = PPU.BG[b].SCBase;
      fr_bg_namebase[b] = PPU.BG[b].NameBase;
      fr_bg_scsize[b] = PPU.BG[b].SCSize;
      fr_bg_tile16[b] = PPU.BG[b].BGSize;
   }
   fr_bgmode = PPU.BGMode;
   fr_bg3prio = PPU.BG3Priority;
   fr_fblank = PPU.ForcedBlanking ? 1 : 0;
   ...
   fr_objlines = s_pub_ol;
   fr_objw = s_pub_ow;
   fr_objvis = s_pub_ov;
   fr_objs = s_pub_ob;
   fr_objnamebase = s_pub_onb;
   fr_objnamesel = s_pub_ons;
   fr_clip = s_pub_clip;
   for (b = 0; b < 4; b++)
   {
      fr_bg_scbase[b] = s_pub_bgw[b];
      fr_bg_namebase[b] = s_pub_bgw[4 + b];
      fr_bg_scsize[b] = s_pub_bgw[8 + b];
      fr_bg_tile16[b] = s_pub_bgb[b];
   }
   fr_bgmode = s_pub_bgb[4];
   fr_bg3prio = s_pub_bgb[5];
   fr_fblank = s_pub_rg[5];
   GFX.r212c = s_pub_rg[0];
   GFX.r212d = s_pub_rg[1];
   GFX.r2130 = s_pub_rg[2];
   GFX.r2131 = s_pub_rg[3];
   GFX.Pseudo = s_pub_rg[4] != 0;
   return s_pub_sh;
#endif
}

const SLineData *snes_worker_ld(void) { return s_pub_ld; }
const uint32_t *snes_worker_fx(void) { return s_pub_fx; }
...
/* MESHPUNK: the output buffers hold 256 pixels per row (GFX.RealPitch
 * bytes). The stock hires layout (BG modes 5/6, interlace) writes 512-wide
 * rows and overruns them. This build renders hires at half width through
 * the port's half-width tile drawers instead: BG modes 5/6 sample every
 * other hires pixel into a normal 256-wide row. Called at frame start and
 * before every drawn span, so mid-frame mode switches re-select the
 * renderers without any migration of already-drawn rows. */
static INLINE void SelectHiResMode(void)
{
...
{
   memcpy(snes_frame_pal, IPPU.ScreenColors, sizeof(snes_frame_pal));
   snes_pal_j_count = 0;
   snes_mode_mask = 0;
   snes_mid_mask = 0;
   snes_cur_line = 0;
   ...
   if (IPPU.RenderThisFrame)
   {
      ...
      /* MESHPUNK: one 256-wide single-field layout for every mode; BG
       * modes 5/6 go through the half-width renderers (SelectHiResMode).
       * IPPU.Interlace stays false — DrawBackgroundMode5 doubles its Y
       * range when it is set, which the single-field buffers cannot hold. */
      SelectHiResMode();
...
{
   /* MESHPUNK: the journals record every frame — worker-rendered frames
    * run with RenderThisFrame false (CurrentLine stays 0, FLUSH_REDRAW
    * inert) and consume the published copies. snes_cur_line carries the
    * line number that IPPU.CurrentLine cannot for those frames. */
   snes_cur_line = C;
   snes_mode_mask |= (uint8_t)(1 << (PPU.BGMode & 7));

   LineData[C].BG[0].VOffset = PPU.BG[0].VOffset + 1;
   LineData[C].BG[0].HOffset = PPU.BG[0].HOffset;
   LineData[C].BG[1].VOffset = PPU.BG[1].VOffset + 1;
   LineData[C].BG[1].HOffset = PPU.BG[1].HOffset;
   snes_fixedc_j[C] = (uint32_t)PPU.FixedColourRed
                    | ((uint32_t)PPU.FixedColourGreen << 8)
                    | ((uint32_t)PPU.FixedColourBlue << 16);
...
/* MESHPUNK: counts frames that modified SRAM. CPU.SRAMModified is cleared
 * below every frame, inside S9xMainLoop — the module's main loop only runs
 * between frames, so it watches this counter for the .srm autosave. */
volatile uint32_t snes_sram_dirty = 0;

void S9xEndScreenRefresh(void)
{
   /* Cleared before the publish below so V-blank palette writes (whole-
    * palette updates in the NMI handler) are not journaled as line 223. */
   snes_in_active = 0;
   ...
   /* MESHPUNK: hand the render worker this frame's display state at the
    * exact point the stock renderer drew it (start of V-blank, before the
    * game's NMI handler uploads next-frame graphics). No-op unless a
    * worker is waiting for a frame. */
   snes_worker_publish();
}
...
void S9xSetupOBJ(void)
{
   ...
   snes_obj_dirty = 1; /* MESHPUNK: sprite tables changed — republish */
...
static void DrawOBJS_impl(bool OnMain, uint8_t D)
{
   ...
   /* MESHPUNK: sprites sit on the 256-pixel OBJ grid in every mode. The
    * half-width hires rows are 256 wide too, so sprite pixels map 1:1 and
    * the normal renderers apply everywhere; the x2/x2x2 variants target
    * the 512-wide layout this build does not use (GFX.PixSize stays 1). */
   DrawTilePtr = DrawTile16;
   DrawClippedTilePtr = DrawClippedTile16;
...
void S9xUpdateScreen(void)
{
   ...
   /* MESHPUNK: rows are always 256 wide (half-width hires), so a mid-frame
    * BG-mode switch only needs the renderers re-selected for the span
    * about to be drawn — already-rendered rows keep their layout and no
    * pixel migration happens in either direction. x2 stays 1: the clip
    * spans below are in SNES pixel units. */
   SelectHiResMode();
   ...
   if (snes_fast_ok())
   {
      fr_vram = Memory.VRAM;
      fr_pal = IPPU.ScreenColors;
      /* Live path: CGRAM writes flush the span, so the palette bound here
       * is already correct for the lines about to be drawn. */
      fr_pal_j = NULL;
      fr_pal_jn = 0;
      fr_fblank = PPU.ForcedBlanking != 0;
      fr_objlines = GFX.OBJLines;
      fr_objw = GFX.OBJWidths;
      fr_objvis = GFX.OBJVisibleTiles;
      fr_objs = PPU.OBJ;
