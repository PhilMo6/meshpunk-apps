...
{
   /* Records PPU state rewritten once rendering is under way; see ppu.h.
    * snes_cur_line is 0 until the first line renders, which excludes
    * ordinary frame setup — a change on line 0 applies to the whole frame
    * and needs no split anyway. */
   if (snes_in_active && snes_cur_line)
   {
      /* $2100 shares bit 0 with $2105: both are screen configuration the
       * Core-1 renderer binds once per frame. $2100 carries the forced-blank
       * bit, and a mid-frame toggle would otherwise black a whole frame. */
      if (Address == 0x2105 || Address == 0x2100)       snes_mid_mask |= 0x01;
      else if (Address >= 0x2107 && Address <= 0x210c)  snes_mid_mask |= 0x02;
      else if (Address == 0x2122 || Address == 0x2121)  snes_mid_mask |= 0x04;
      else if (Address >= 0x2123 && Address <= 0x212b)  snes_mid_mask |= 0x08;
      else if (Address == 0x212c || Address == 0x212d)  snes_mid_mask |= 0x10;
      else if (Address == 0x2130 || Address == 0x2131)  snes_mid_mask |= 0x20;
      else if (Address == 0x2118 || Address == 0x2119)  snes_mid_mask |= 0x40;
      else if (Address == 0x2104 || Address == 0x2102 ||
               Address == 0x2103 || Address == 0x2101)  snes_mid_mask |= 0x80;
   }
   ...
   if (Address <= 0x2183)
   {
      ...
      case 0x2105: /* Screen mode (0 - 7), background tile sizes and background 3 priority */
         if (Byte != Memory.FillRAM [0x2105])
         {
            ...
            /* MESHPUNK: IPPU.Interlace stays false — output is single-field
             * 256-wide; DrawBackgroundMode5 doubles its Y range when set. */
         }
         break;
      ...
      case 0x2132:
         ...
         {
            /* MESHPUNK: no FLUSH_REDRAW — the fixed colour is journaled
             * per line by RenderLine (snes_fixedc_j) and consumed by the
             * fast renderer, so HDMA colour gradients no longer fragment
             * the frame into one-line spans. */
         ...
         if (Byte != Memory.FillRAM [0x2133])
         {
            ...
            if ((Memory.FillRAM [0x2133] ^ Byte) & 3)
            {
               ...
               if ((Memory.FillRAM [0x2133] ^ Byte) & 2)
                  IPPU.OBJChanged = true;
               /* MESHPUNK: IPPU.Interlace stays false (single-field output). */
            }
         }
         break;
      case 0x2134:
   ...
   else
   {
      ...
      else if (Address >= 0x3000 && Address < 0x3300)
      {
          ...
          * here, which left the GSU inert — the 65816 side of an FX cart
          * runs (sound, input, game loop) but nothing ever renders.
          * Writing $3030 with the GO flag set starts the chip; the read
          * side in S9xGetPPU was already intact. */
         Memory.FillRAM[Address] = Byte;
         if (Settings.SuperFX)
         {
            /* Writing the high byte of R15 loads the GSU's program counter
             * and starts it: the chip sets its own GO flag. Yoshi's Island
             * configures SCMR/SCBR/PBR and then writes $301e/$301f — with
             * only $3030 watched here that start signal was discarded, so
             * the register file filled in correctly and nothing ever ran. */
            /* Set GO only. Running the chip inline here starves the 65816:
             * the game polls by rewriting R15, and every poll then costs a
             * full scanline of GSU instructions, so H-blank processing is
             * never reached and emulation stops advancing. The per-scanline
             * hook in cpuexec.c is what drives execution. */
            if (Address == 0x301f)
