...
#include "snes9x.h"
#include "memmap.h"
/* MESHPUNK: this port has no getset.h and fxemu.c references nothing from
 * it — the GSU reaches memory through its own pvRom/pvRam pointers. ppu.h
 * replaces it here for GSU_IRQ_SOURCE. */
#include "ppu.h"
#include "cpuexec.h"
#include "fxinst.h"
#include "fxemu.h"

/* MESHPUNK: snes9x kept both of these in fxinst.c. snes9x2010 folded that
 * file's opcode bodies into this one but left the GSU state undefined, and
 * it spells the IRQ helper differently from this port. */
struct FxRegs_s GSU;
#define S9X_SET_IRQ(s) S9xSetIRQ(s)
...
void S9xResetSuperFX (void)
{
   ...
   /* Overclock. The budget is what the game's wait loops are timed against,
    * so raising it retires a GSU job in fewer scanlines and the game spends
    * fewer frames waiting. Unlike FX_BLANK_BOOST this applies on every line,
    * not only blanked ones, which is what a game that never forced-blanks
    * while it waits needs. */
   SuperFX.speedPerLine = (uint32_t) ((uint64_t) SuperFX.speedPerLine
                                      * FX_SPEED_PCT / 100);
...
/* Off: the per-opcode cost model reads fx_OpcodeCycles[idx] from PSRAM and
 * does the budget arithmetic on every instruction. Both executors dispatch
 * through FX_DISPATCH, so this changes the budget model only. */
int		fx_cycle_accuracy = 0;
...
/* MESHPUNK: the fx_OpcodeTable entries a GSU-heavy game spends most of its
 * dispatches in, measured. Calling them directly drops the table load and the
 * indirect call, and lets the compiler inline the small ones into the loop,
 * which runs from internal SRAM. ALT0 only: idx carries the ALT bits, so none
 * of these can alias an ALT1/2/3 opcode. 0x04c is absent on purpose --
 * fx_OpcodeTable[0x04c] is repointed at run time by vMode. Shared by both
 * executors so the cycle-accuracy setting changes only the budget model. */
#define FX_DISPATCH(idx) \
	switch (idx) \
	{ \
		case 0x001: fx_nop();     break; \
		case 0x003: fx_lsr();     break; \
		case 0x004: fx_rol();     break; \
		case 0x005: fx_bra();     break; \
		case 0x006: fx_bge();     break; \
		case 0x007: fx_blt();     break; \
		case 0x008: fx_bne();     break; \
		case 0x009: fx_beq();     break; \
		case 0x00a: fx_bpl();     break; \
		case 0x00b: fx_bmi();     break; \
		case 0x00c: fx_bcc();     break; \
		case 0x00d: fx_bcs();     break; \
		case 0x00e: fx_bvc();     break; \
		case 0x00f: fx_bvs();     break; \
		case 0x031: fx_stw_r1();  break; \
		case 0x03c: fx_loop();    break; \
		case 0x03d: fx_alt1();    break; \
		case 0x03e: fx_alt2();    break; \
		case 0x03f: fx_alt3();    break; \
		case 0x0d0: fx_inc_r0();  break; \
		case 0x0d1: fx_inc_r1();  break; \
		case 0x0d2: fx_inc_r2();  break; \
		case 0x0d3: fx_inc_r3();  break; \
		case 0x0d4: fx_inc_r4();  break; \
		case 0x0d5: fx_inc_r5();  break; \
		case 0x0d6: fx_inc_r6();  break; \
		case 0x0d7: fx_inc_r7();  break; \
		case 0x0d8: fx_inc_r8();  break; \
		case 0x0d9: fx_inc_r9();  break; \
		case 0x0da: fx_inc_r10(); break; \
		case 0x0db: fx_inc_r11(); break; \
		case 0x0dc: fx_inc_r12(); break; \
		case 0x0dd: fx_inc_r13(); break; \
		case 0x0de: fx_inc_r14(); break; \
		default:    (*fx_OpcodeTable[idx])(); break; \
	}

/* MESHPUNK: the dispatch loop is the GSU's whole cost — roughly a million
 * instructions a second on an FX game, each one a table load and an indirect
 * call into an interpreter spread across ~100KB of PSRAM, which the 16KB
 * shared instruction cache cannot hold. Running the loop from internal SRAM
 * takes no cache miss and evicts nothing. The build gives this file
 * -mtext-section-literals so l32r targets stay inside the relocated section. */
...
void S9xSuperFXExec (void)
{
	...
	/* The chip executes only with GO set and both buses granted. Returning
	 * here, ahead of fx_readRegisterSpace/fx_writeRegisterSpace, leaves the GSU
	 * registers untouched: a job in progress keeps its GO flag and R15 while the
	 * game holds the ROM bus, and resumes on the line the bus is granted back. */
	if (!CHECK_EXEC_SUPERFX())
		return;

	/* EMULATE FX CHIP*/
	/* Execute until the next stop instruction*/
	nInstructions = (Memory.FillRAM[0x3000 + GSU_CLSR] & 1) ? SuperFX.speedPerLine * 2 : SuperFX.speedPerLine;
