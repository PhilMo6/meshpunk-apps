...
/* MESHPUNK: per-sample helper -- always_inline (NOT tagged MESHPUNK_HOT, which
   would stop GCC inlining it and put a real call in the sample loop); folded
   into YM3812UpdateOne it lands in .iram.text with its caller. */
static MESHPUNK_INLINE uint32_t OPL_CALC_SLOT( OPL_SLOT *SLOT )
{
...
/* ---------- update one of chip ----------- */
MESHPUNK_HOT void YM3812UpdateOne(FM_OPL *OPL, int16_t *buffer, int length)
{
	int i;
	int data;
	int16_t *buf = buffer;
