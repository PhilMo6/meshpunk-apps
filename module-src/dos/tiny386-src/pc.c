...
#else
/* MESHPUNK: both were hard-coded here for desktop builds. The T-Deck module is
   neither desktop nor their ESP-IDF app, so make them -D overridable:
   PC_STEP_COUNT bounds how long pc_step() runs before our loop can feed the
   watchdog (their ESP port uses 512, the desktop value 10240 is far too long),
   and MIXER_BUF_LEN sizes a stack buffer inside mixer_callback, which runs on
   the firmware's sound task rather than ours. See VENDOR.md. */
#ifndef MIXER_BUF_LEN
...
/* MESHPUNK: one-shot unhandled-port logging. A game that polls an unimplemented
   port (MS Pac-Man hits the joystick port 0x201 in a loop) would otherwise emit
   one serial write per access, which dominates emulation time on this hardware.
   The bitmap covers the low 1K of I/O space, which is where everything a DOS
   program touches lives; higher ports fold into the same slots and simply share
   a one-shot. */
static uint8_t meshpunk_port_logged[1024 / 8];

static void meshpunk_log_unhandled_port(const char *op, int addr, int val)
{
	unsigned slot = (unsigned)addr & 0x3FF;
	uint8_t bit = (uint8_t)(1u << (slot & 7));
	if (meshpunk_port_logged[slot >> 3] & bit)
		return;
	meshpunk_port_logged[slot >> 3] |= bit;
	fprintf(stderr, "%s 0x%x => 0x%x (unhandled; further writes silent)\n",
		op, val, addr);
}

/* MESHPUNK: only synthesise the Adlib once the guest has actually touched it.
   Measured on hardware: MS Pac-Man never accesses a single FM port, yet
   adlib_callback()/YM3812UpdateOne() ran on every mixer slice producing
   ~40k samples/s of silence. Gated on ANY access, read or write -- detection
   code polls the status port for the OPL timers, so a guest that only reads
   must still get a live chip. Once set it stays set; a guest that silences
   the OPL and stops writing keeps being synthesised, which is correct. */
static int meshpunk_adlib_touched = 0;
...
static void pc_io_write(void *o, int addr, u8 val)
{
	...
	case 0xf1f4:
		...
		return;
	default:
		/* MESHPUNK: log each unhandled port ONCE. This fired on every write,
		   and a game polling the joystick port (0x201) turns that into a
		   USB-serial write per poll, which costs far more than the emulated
		   instruction. Upstream already commented out the equivalent on the
		   read path. Keeping a one-shot preserves the diagnostic. */
		meshpunk_log_unhandled_port("out", addr, val);
		return;
	}
}
...
static void pc_io_write16(void *o, int addr, u16 val)
{
	...
	case 0x310:
		...
		return;
	default:
		meshpunk_log_unhandled_port("outw", addr, val);   /* MESHPUNK: see above */
		return;
	}
}
...
static void pc_io_write32(void *o, int addr, u32 val)
{
	...
	case 0xf1f4:
		...
		return;
	default:
		meshpunk_log_unhandled_port("outd", addr, val);   /* MESHPUNK: see above */
		return;
	}
}
...
void pc_step(PC *pc)
{
	...
	/* MESHPUNK: the two i8257_dma_run() calls that lived here now run on the
	   firmware's sound task (Core 1) via pc_audio_dma_pump() -- see below. */
	   ...
	   u8 *fb, PCConfig *conf)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		/* MESHPUNK: was assert(ret == 0), which aborted the whole module.
		   The attach fails for an unreadable file OR for any image whose
		   size is not one of the standard floppy geometries -- and picking a
		   HARD DISK image in the A: slot is a normal user mistake in the
		   launcher (both slots list every .img). Leave the drive empty and
		   carry on: SeaBIOS then just boots the next device, which is
		   usually exactly what the user meant. */
		if (ret != 0)
			printf("[dos] drive %c: cannot use %s -- not a floppy image "
			       "(wrong slot?); leaving it empty\n", 'A' + i, fdd[i]);
	}
...
/* MESHPUNK: audio DMA, moved off the emulation core.
 *
 * The SoundBlaster is the ONLY DMA user in this machine -- both
 * i8257_dma_register_channel() calls are in sb16.c, and the floppy interface
 * (emulink) does not use DMA at all. So servicing the DMA controller is purely
 * audio work, and it used to run in pc_step() on the emulation core: every
 * step, and when the guest was playing samples it copied them out of guest RAM
 * right there. Turning audio off measurably sped the emulator up, because with
 * no drain callback running, SB_read_DMA's `audio_free` never refreshed and the
 * copying stopped.
 *
 * The module now calls this from its audio pull callback instead, which the
 * firmware runs on the sound task on Core 1. Besides taking the copy off the
 * emulation core, it REMOVES a race the original code documented and accepted:
 * SB_read_DMA (producer) ran on Core 0 while sb16_audio_callback (consumer) ran
 * on Core 1, both touching audio_p/audio_q. Driven from here they are the same
 * thread.
 *
 * Still racy by design, and bounded: the guest can reprogram the DMA registers
 * from Core 0 while a transfer reads them. i8257_dma_read_memory() bounds every
 * access against phys_mem_size, so the worst case is a wrong-but-valid read --
 * an audio glitch, not a fault. The port-write handlers in i8257.c also call
 * i8257_dma_run() directly; its `running` flag makes that self-consistent, and
 * the regular pump from here picks up anything skipped.
 */
void pc_audio_dma_pump(PC *pc)
{
	i8257_dma_run(pc->isa_dma);
	i8257_dma_run(pc->isa_hdma);
}

MESHPUNK_HOT void mixer_callback (void *opaque, uint8_t *stream, int free)
{
	...
	/* MESHPUNK: skip the whole FM path -- synth AND its mix pass -- for a
	   guest that has never touched the chip. The mix pass alone was adding
	   a known-zero tmpbuf to every sample. */
	if (meshpunk_adlib_touched) {
