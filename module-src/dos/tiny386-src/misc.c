...
#ifndef BUILD_ESP32
#if defined(MESHPUNK_NO_TTY)
/* MESHPUNK: the T-Deck module owns the screen and keyboard and runs with the
   guest serial port disabled (conf.enable_serial = 0), so there is no host
   console to capture. Stub the three tty helpers the U8250 code calls; this
   also keeps <sys/ioctl.h>/<termios.h>/<signal.h> out of the module build,
   none of which exist in the ELF module's libc surface. See VENDOR.md. */
void CaptureKeyboardInput(void) {}
static int ReadKBByte(void) { return -1; }
